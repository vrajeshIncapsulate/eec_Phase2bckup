        ...
        try {
            if (q.Priority == 'high' && (q.Cases == null || q.Cases.size() == 0) && Trigger.oldMap.get(q.id).Priority != 'high') {
                ...
                if (Test.isRunningTest()) {
                  List<Account> testA = [Select Provider_City__c From Account Where Id = :q.CreatedBy.AccountId LIMIT 1];
                  System.debug('----> TestA: ' + testA);
                  if (testA == null || testA.size() == 0) {
                    newCase.Unlicensed_Provider_City__c = EEC_TestUtil.mockCity().Id;  
                    newCase.Unlicensed_Provider_Name__c = 'Test Unlicensed Provider';
                    newCase.Unlicensed_Provider_Zip_Code__c = '12345';
                  }
                    
                  if (testA != null && testA.size() > 0 && testA[0].Provider_City__c == null) {
                    newCase.Unlicensed_Provider_City__c = EEC_TestUtil.mockCity().Id;            
                    newCase.Unlicensed_Provider_Name__c = 'Test Unlicensed Provider';
                    newCase.Unlicensed_Provider_Zip_Code__c = '12345';
                  }
            ...
            mail.setToAddresses(new String[] { (Test.isRunningTest() ? 'test@test.com' : Site.getAdminEmail()) });
